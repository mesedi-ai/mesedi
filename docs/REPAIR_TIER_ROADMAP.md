...
**Mesedi v1** (currently in-flight, local-only posture, post-LOI launch) — no repair tier yet. The story is "detect, cluster, optionally stop, escalate, drop-in adopt." All seven failure-class detectors, failure-group dedup, hard-halt with local budgets plus SSE remote channel, dashboard with collapse-by-class view, dogfood substrate, operator Halt button, continuous synthetic-org traffic via launchd, **webhook escalation on first-occurrence of a failure_group with HMAC-signed payloads + retry/backoff + per-project class filter + dashboard UI**, **framework adapters for LangChain (Python) + CrewAI (Python) + Vercel AI SDK (TypeScript) that one-line attach Mesedi telemetry to existing agent code**. Remaining v1 work: docs and quickstart polish, then production deployment (Mesedi GitHub org + npm + PyPI publication).
