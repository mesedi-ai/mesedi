"""LangGraph integration for Mesedi (Mesedi #26).
...
What the integration emits:
  ...
  - ``checkpoint`` events at every node entry. The node name lands
    ...
    semantic_loop detector (#6) can catch agents that revisit the
  ...
  - ``agent_handoff`` events when the graph invokes a compiled
    ...
    or "spawn" (Send-style fan-out). Feeds cascading_failure (#12)
    and coordination_deadlock (#13) when the sub-graph runs as a
    nested @wrap.
...
class MesediLangGraphHandler(MesediCallbackHandler):
    ...
    state. The semantic_loop detector (#6) hashes this state to
    ...
    (#11) so the topology graph (#10) can render the parent/child
    relationship and the cascading_failure (#12) +
    coordination_deadlock (#13) detectors can fire on the resulting
    cross-node patterns.
