...
What the hooks emit:
  ...
  - ``checkpoint`` events at every agent start AND end. The agent
    ...
    Feeds the semantic_loop detector which hashes canonical
    state.
  - ``agent_handoff`` events on every handoff between agents. The
    ...
    Feeds cascading_failure and coordination_deadlock.
...
class MesediRunHooks(RunHooks):
    ...
    async def on_agent_start(self, context: Any, agent: Any) -> None:
        ...
        repr of the runner context. The semantic_loop detector
    ...
    async def on_handoff(
        ...
        ``agent_handoff`` event so the topology graph renders
        the parent/child relationship and the cascading_failure
 + coordination_deadlock detectors fire on the
