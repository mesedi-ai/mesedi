...
# emit_mcp_call moved to mesedi.mcp on 2026-09-10; re-exported here so
# `from mesedi.observe import emit_mcp_call` keeps working.
from mesedi.mcp import emit_mcp_call  # noqa: F401,E402 -- re-export so existing imports keep working after the move
...
def emit_eval_score(
    ...
    and want Mesedi to track the score over time. 
...
def emit_agent_handoff(
    ...
    . Use this at the moment one agent invokes another:
    ...
    topology graph so that the cascading_failure detector
 can correlate a handoff with a child execution that
    ...
    * ``"consult"``: short Q&A, return text only
    ...
    Resolution of ``from_agent`` (ergonomics):
...
def resume_for_agent() -> None:
    ...
    ``awaiting_human`` back to ``started`` ().
...
class HumanInterventionHandle:
    """Handle to an in-flight HITL request.
    ...
    def complete(
        ...
        HITL detectors (, ) only recognize the five
        well-known values.
        """
...
def request_human_intervention(
    ...
    """Pause the current execution and return a handle.
    ...
    own SLA expectation. The hitl_timeout detector reads
    this to fire when the actual wait exceeds the configured SLA.
    Customers without an explicit SLA can omit it; will then
