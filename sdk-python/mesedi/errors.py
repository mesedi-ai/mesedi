...
class ErrorClass:
    ...
    Defined as string constants rather than an enum so the values
    serialize directly into JSON payloads without any
    enum-to-string ceremony.
    """

    # Provider says you're going too fast. Maps to HTTP 429 in most
    # provider APIs.
    RATE_LIMITED = "rate_limited"

    # You hit a billing / quota cap on the provider side. Distinct
    # from RATE_LIMITED because remediation differs (raise quota
    # vs. back off).
    QUOTA_EXHAUSTED = "quota_exhausted"

    # Provider had an internal 5xx failure. Includes malformed
    # provider responses that fail our SDK's own response
    # validation.
    ...
    # bug, not a provider incident — the backend's
    # provider_incident detector filters this class out.
...
# integrations stay consistent without a backend round trip.
PROVIDER_SIDE_ERROR_CLASSES = frozenset(
    {
