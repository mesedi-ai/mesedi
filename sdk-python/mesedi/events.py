...
class EventType:
    ...
    #  transport-plane backpressure (HTTP 429, circuit
    ...
    #  peer to TOOL_CALL for Model Context Protocol server
    ...
    #  external evaluator score (Ragas / Promptfoo / HHEM
    # / custom judge). Ingestion-only; aggregates these
    # into grounding_failure clusters.
    EVAL_SCORE:           ClassVar[str] = "eval_score"
    #  external memory store read/write/search.
    MEMORY_OPERATION:     ClassVar[str] = "memory_operation"
    #  cross-agent task delegation. Emitted when one
    # agent hands off work to another (supervisor/worker,
    # plan/execute, role-based multi-agent). The downstream
    # cascading_failure detector correlates the handoff with
    # the resolved child execution via the topology graph.
    AGENT_HANDOFF:        ClassVar[str] = "agent_handoff"
    #  full HITL ask/answer cycle. Emitted at the moment
