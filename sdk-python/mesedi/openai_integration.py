...
def instrument_openai(
    ...
    emit llm_call events. Patches both sync + async surfaces (
    ...
    # : async-aware patching. Mirrors the sync patchers but
    ...
    # : embeddings surface — non-chat but provider_incident-
    ...
    # sub-ship 2: image + audio surfaces. Each one is fail-open
...
def _patch_chat_completions(cls: Type[Any]) -> None:
    ...
    def patched_create(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            # exceptions so infrastructure_throttled isn't
        ...
        # : stream=True → response is a Stream iterator, not a
...
def _patch_responses(cls: Type[Any]) -> None:
    ...
    def patched_create(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            # Auto-emit infrastructure_event on throttling-class
            # exceptions; same rationale as the chat
        ...
        # : stream=True → wrap iterator for chunk aggregation.
        if kwargs.get("stream") is True:
            return _OpenAIStreamIteratorWrapper(
                inner=response,
                ctx=ctx,
...
def _patch_async_chat_completions(cls: Type[Any]) -> None:
    ...
    async def patched_create(self: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        # : stream=True → wrap async iterator for chunk aggregation.
        if kwargs.get("stream") is True:
...
def _build_failure_event(
    *,
    ...
    The ``surface`` field (added 2026-06-26 in ) distinguishes
...
## ── sub-ship 2 — image + audio surfaces ────────────────────────
