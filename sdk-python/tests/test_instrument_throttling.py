"""Unit tests for — auto-emit infrastructure_event from
instrument_* modules.
...
# which is the level of testing 's classify_*_exception tests
...
def test_each_integration_module_imports_the_helper() -> None:
    ...
    for mod in (
        ...
        assert hasattr(mod, "_maybe_emit_throttling_event"), (
            ...
            "the auto-emit will silently no-op for this provider."
        )


def test_ollama_integration_does_NOT_import_throttling_helper() -> None:
    """— instrument_ollama must NOT import
    _maybe_emit_throttling_event. Ollama is a local runtime: no
    per-minute rate limiting, no quota exhaustion. 's
