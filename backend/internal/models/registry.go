...
//
//   - context_overflow (Mesedi item ): compares cumulative tokens
//     across an execution against the configured model's context
//     window, fires at WARN @ 90% and FAIL @ 100%.
//
//   - token_waste (Mesedi item ): uses the context window as one
...
// like provider_incident and for routing rate-limit data to
...
var windowByModel = map[string]int{
	...
	// prompt in makes the override path discoverable.
...
var providerByModel = map[string]string{
	...
	"command":                "cohere",
	// Ollama (local runtime; ). Provider="ollama" matches
