...
The detector accepts per-project custom regex patterns. Custom matches surface under `sandbox_escape:custom:<pattern_id>`. Built-in patterns run before custom patterns; both contribute to the per-execution match cap of 20. Use custom patterns for vectors the built-ins miss (e.g. `sudo`/`su -` for stacks where those need to be flagged, JavaScript dynamic `import()`, Ruby `Kernel.system`, obfuscated base64 payloads that decode to known dangerous shapes).
...
Stated plainly, decided 2026-09-10 and revised 2026-09-15: this detector sees what arrives inside `tool_call` arguments and return values, and nothing below that line. The schema now carries an `egress` event type, reopened after three trace-edge incidents in one fortnight, and it narrows this boundary without removing it: egress events are REPORTED by the host application or sandbox, never inferred, so a connection opened by already-running code that nobody reports is exactly as invisible as before. There is still no filesystem-write event type, so a file written inside the sandbox, and the technique where an escape is assembled from pieces that individually look benign, remain outside what any of this can see. If your sandbox surfaces syscall-level activity, forward it through tool events or egress events and the patterns and counters apply; if it does not, this detector is not your syscall monitor and should not be treated as one.
