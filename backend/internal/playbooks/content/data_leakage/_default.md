...
Mesedi's DLP scanner redacts the matched value at event ingest, before the event reaches durable storage. The string `[REDACTED:rule_id]` replaces the original (built-in rules) or `[REDACTED:custom-<pattern_id>]` (custom rules — Wave 2.1.d.3 closed the gap where custom rules detected but didn't redact). If you open the event in the dashboard you will see the redaction marker, not the secret. That means the secret is not at rest on Mesedi's side; the issue to remediate is the leak at the source.
