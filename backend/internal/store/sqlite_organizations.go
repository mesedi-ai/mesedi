...
// Team / multi-seat store methods, SQLite implementation (#263).
// Three concerns in one file: organizations, organization_members,
// organization_invites. Sidecar to keep sqlite.go uncluttered.
...
func (s *SQLiteStore) GetOrganization(ctx context.Context, orgID string) (*Organization, error) {
...
func (s *SQLiteStore) UpdateOrganizationName(ctx context.Context, orgID, name string) error {
	if name == "" {
...
func (s *SQLiteStore) ListOrganizationsForUser(ctx context.Context, userID string) ([]*Organization, error) {
...
func (s *SQLiteStore) AddOrganizationMember(ctx context.Context, m *OrganizationMember) error {
	...
	`, m.OrgID, m.UserID, m.Role, nullableString(m.Email), nullableString(m.AddedByUserID), now)
	if err != nil {
...
func (s *SQLiteStore) ListOrganizationMembers(ctx context.Context, orgID string) ([]*OrganizationMember, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT org_id, user_id, role, email, added_by_user_id, added_at
		FROM organization_members
		WHERE org_id = ?
	...
	for rows.Next() {
		m, scanErr := scanOrganizationMemberRows(rows)
		if scanErr != nil {
			return nil, scanErr
		}
		out = append(out, m)
...
func (s *SQLiteStore) UpdateOrganizationMemberRole(ctx context.Context, orgID, userID, newRole string) error {
	res, err := s.db.ExecContext(ctx, `
...
func (s *SQLiteStore) RemoveOrganizationMember(ctx context.Context, orgID, userID string) error {
	res, err := s.db.ExecContext(ctx, `
...
func scanOrganizationMember(row *sql.Row) (*OrganizationMember, error) {
...
func scanOrganizationMemberRows(rows *sql.Rows) (*OrganizationMember, error) {
	m := &OrganizationMember{}
	var email, addedBy sql.NullString
	var addedAt int64
...
func (s *SQLiteStore) CreateOrganizationInvite(ctx context.Context, inv *OrganizationInvite) error {
...
func (s *SQLiteStore) GetOrganizationInviteByToken(ctx context.Context, token string) (*OrganizationInvite, error) {
	row := s.db.QueryRowContext(ctx, `
		SELECT invite_id, org_id, email, role, token,
		       invited_by_user_id, created_at, expires_at,
		       accepted_at, accepted_by_user_id
		FROM organization_invites WHERE token = ?
	`, token)
	return scanOrganizationInvite(row)
}

func (s *SQLiteStore) ListOrganizationInvites(ctx context.Context, orgID string, pendingOnly bool) ([]*OrganizationInvite, error) {
	query := `
	...
	args := []any{orgID}
	if pendingOnly {
		query += " AND accepted_at IS NULL AND expires_at > ?"
	...
	for rows.Next() {
		inv, scanErr := scanOrganizationInviteRows(rows)
		if scanErr != nil {
			return nil, scanErr
		}
		out = append(out, inv)
...
func (s *SQLiteStore) MarkInviteAccepted(ctx context.Context, inviteID, acceptedByUserID string) error {
	now := time.Now().UTC().Unix()
	// Race-safe single-use: WHERE accepted_at IS NULL ensures we
	// only flip pending invites. RowsAffected == 0 means either
	// the invite doesn't exist OR it was already accepted; we
	...
	n, _ := res.RowsAffected()
	if n == 0 {
		// Disambiguate: was it not found, or already accepted?
		var exists int
		if qerr := s.db.QueryRowContext(ctx, `
			SELECT COUNT(*) FROM organization_invites WHERE invite_id = ?
...
func (s *SQLiteStore) RevokeOrganizationInvite(ctx context.Context, inviteID, orgID string) error {
	res, err := s.db.ExecContext(ctx, `
		DELETE FROM organization_invites
		WHERE invite_id = ? AND org_id = ? AND accepted_at IS NULL
	`, inviteID, orgID)
	if err != nil {
...
func scanOrganizationInvite(row *sql.Row) (*OrganizationInvite, error) {
...
func scanOrganizationInviteRows(rows *sql.Rows) (*OrganizationInvite, error) {
	inv := &OrganizationInvite{}
	var createdAt, expiresAt int64
	var acceptedAt sql.NullInt64
	var acceptedBy sql.NullString
...
func (s *SQLiteStore) ListProjectsByTenant(ctx context.Context, tenantID string) ([]*Project, error) {
	if tenantID == "" {
		return []*Project{}, nil
	}
	rows, err := s.db.QueryContext(ctx, `
