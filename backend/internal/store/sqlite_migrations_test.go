// Regression test for #233: SQLite fresh-install migration sequence
...
// #231's store-test setup tried to spin up a fresh SQLite database
...
func TestSQLiteFreshInstallRunsAllMigrations(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("OpenSQLite on a fresh db must succeed (#233 fresh-install path); got: %v", err)
	}
	t.Cleanup(func() { _ = s.Close() })

	// project_webhooks is what was missing pre-#233. If 003 was lost
