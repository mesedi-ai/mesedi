...
const (
	...
	//
	// .F (closes / ): cut from 10000 to 5000 to
	...
	// period. The cap is per-organization (tenant_id),
	...
	// the operator's account; without a cap, an org generating
	...
	// the pay-per-use model (product decision). Past this
	...
	// logs).
	//
	// .F (closes / ): cut from 50 to 25 to halve
	...
	// tier (product decision). Billed via the existing
	...
	// included in the flat $99/mo (product decision).
	//
...
//
// SecretKeyTest (follow-on): optional test-mode secret API key
...
//
// WebhookSecretTest: optional second signing secret for a
...
// Empty / unset = behavior identical to the single-secret
...
// configured. Returns the parsed event, a short label
...
func constructStripeEvent(body []byte, sig string, liveSecret, testSecret string) (stripe.Event, string, error) {
	...
	// scraping tooling sees the same message shape it did .
	return event, "", err
}
...
// the event's livemode (follow-on). When livemode=true, uses
...
// per ) is hard-capped at the included quota. The first execution
...
func capExceeded(p *store.Project) (bool, float64, float64) {
	...
	// theirs mid-cycle. Reads card_on_file (migration 022) so
...
// Surfaces how many LLM-assisted root-cause analyses
...
func (h *Handlers) HandleGetBilling(w http.ResponseWriter, r *http.Request) {
	...
	// granted credits (see promo lever). Enterprise has 0 base
	...
	resp := BillingStatusResponse{
		...
		// dashboard wrongly displayed "Card on file" (Robert
...
// spend. Counting follows the same logic as
...
func (h *Handlers) HandleStripeWebhook(w http.ResponseWriter, r *http.Request) {
	...
	// configured. The two-step path lets a single endpoint URL
	...
	// follow-on: swap the package-global stripe.Key to the
...
func (h *Handlers) dispatchStripeEvent(
	...
	case "invoice.payment_failed":
		...
		// dunning case without polling Stripe.
	...
	case "charge.dispute.created":
		...
		// row so /admin/billing-events surfaces the dispute.
...
func (h *Handlers) handleCheckoutCompleted(event stripe.Event) error {
	...
	// CRITICAL (Robert): the SAME checkout.session.completed event
...
func (h *Handlers) handleInvoiceUpcoming(event stripe.Event, logger *slog.Logger) error {
	...
	// : Compute AI analysis overage cost alongside execution
	...
	// ceiling via the dashboard (follow-up).
	//
...
func (h *Handlers) handleSetupIntentSucceeded(event stripe.Event, logger *slog.Logger) error {
	...
	// : Flip card_on_file = TRUE so the cap-check and AI
...
// account flow on /app/settings. Admin only.
func (h *Handlers) HandleDowngradeToHobby(w http.ResponseWriter, r *http.Request) {
	...
	//     customer can actually escape the wrong tier (Robert
	//     flagged this).
	if p.StripeSubscriptionID != "" && h.Stripe.Configured() {
		...
		// Stripe actually agreed to, not what we predicted (email
		// notifications).
		h.sendDowngradeEmailBestEffort(r, p, periodEndFromSub(sub), false)

		// audit log: downgrade is admin-tier and reversible up
...
// the dashboard's 401-logout handler will catch the next
...
func (h *Handlers) HandleCloseAccount(w http.ResponseWriter, r *http.Request) {
	...
	if h.Mailer != nil && h.Mailer.Enabled() && p.OwnerEmail != "" {
		if mailErr := h.Mailer.SendAccountClosed(
			...
			mail.AccountClosedInput{
				...
				// security@, not support@: the body's pointer is
	...
	// audit log: record account-close BEFORE the cascade fires.
	...
	// ships the cleanup job), GDPR purge supported via task
	// . See store/audit_events.go SnapshotAuditEventsForClosedProject.
...
func (h *Handlers) HandleUpdateBillingCap(w http.ResponseWriter, r *http.Request) {
	...
	// audit log: capturing both new value and the project ID
