// Server-to-server sign-in handler (POST /signin, commit 1).
//
...
// listing (design note) so the customer never sees them.
//
...
// with action='sso_login' (or 'magic_link'), actor_email=the
...
type SigninResponse struct {
	...
	// before . As of step 4 (2026-06-16) we only populate it
	...
	// Worker writes onto its browser-facing response (Batch 2).
	...
	// this as the primary success signal post--step-4.
	...
	// NeedsTOTP is true when the user has 2FA enabled and
...
func (h *Handlers) HandleSignin(w http.ResponseWriter, r *http.Request) {
	...
	// 5b. Mark the email verified. SSO callers reach signin
	...
	// 6c. two-factor fork. If this user has TOTP enabled, do NOT
	...
	// 6b. Batch 2: also create a real session row + return its
	...
	//
	//    step 4 (2026-06-16): we no longer return APIKey on the
