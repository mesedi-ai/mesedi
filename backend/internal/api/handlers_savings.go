...
// Mesedi savings: translate the dashboard's failure-group data
...
func (h *Handlers) HandleSavings(w http.ResponseWriter, r *http.Request) {
	...
	// ─────────────────────────────────────────────────────────────
	// Every failure group represents a webhook fired on call of
	...
	resp := savingsResponse{
		...
		Buckets: map[string]savingsBucket{
			...
			"first_occurrence": {
				...
				Assumption: fmt.Sprintf("Each escalation fired on call of the pattern. Without it, the same failure would have recurred %.0f more times before manual detection.", firstOccurrenceMultiplier),
			},
			"budget_ceiling": {
