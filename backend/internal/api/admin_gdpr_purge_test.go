// Unit tests for HandleAdminGDPRPurgeClosedProjectAudit (#219).
//
// Coverage:
...
func Test_HandleAdminGDPRPurge_HappyPath_RecordsMetaAuditEvent(t *testing.T) {
	...
	r := newPurgeRequest(t, "proj-closed-abc", `{"reason":"ticket #4567 verified"}`)
	...
	if md["reason"] != "ticket #4567 verified" {
		t.Errorf("metadata.reason = %v, want \"ticket #4567 verified\"",
			md["reason"])
	}
