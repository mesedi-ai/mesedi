// Webhook escalation auto-fire path (slice 3 recurrence
// modes).
//
...
//
// adds the recurrence path. When the handler calls
...
// recurrence events. Same shape as dispatchFailureGroupCreated
...
func (h *Handlers) runFailureGroupDispatch(
	...
	for _, wh := range hooks {
		...
		// Severity filter. Empty SeverityFilter = fire on
		// every severity (backward compatible with webhooks).
		...
		// recurrence mode gate. Only applies on recurrences;
		...
		// : advance the throttle baseline on every recurrence
