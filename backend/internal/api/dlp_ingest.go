// DLP ingest helpers. When the global DLP scanner
...
var scanFieldKeys = map[events.EventType][]string{
	...
	events.EventTypeToolCall: {
		...
		"arguments",
		"return_value",
		//  tool failure path. `error` is the standard
	...
	//  catch-all for any direct exception_event the SDK
...
func (h *Handlers) applyDLPToBatch(
	...
	//  load per-project custom data-leakage patterns once
...
//
// closes the documented gap from 2.1.b: custom rules
...
func scanAndRedactCustomDLP(
	...
	for _, m := range matches {
		...
		// makes future guard-edit safety obvious by inspection.
		newBuf := make([]byte, 0, len(buf)+len(token))
