...
const (
	...
	// EventTypeAgentHandoff. Emitted when one agent
	...
	// graph to surface cascading_failure and
	// coordination_deadlock.
	EventTypeAgentHandoff EventType = "agent_handoff"
	// EventTypeHumanIntervention. Captures one full
	...
	// detectors (hitl_timeout, hitl_rejection_spike)
...
//  2. Paused: `awaiting_human` (). The agent is suspended,
...
// evaluated yet" rather than "actively making progress" ().
...
type Execution struct {
	...
	// projects. Used by the cost-by-tenant report to break
	...
	// created this execution. Stamped server-side from the auth
	...
	// admin "key recent use" report can scope a forensic audit
	...
	// currently-active human-in-the-loop pause cycle.
	...
	// trip a time_budget alert. The HITL SLA detectors (and
	// ) read this to compute wait duration and pause-cycle
	// frequency.
	...
	// which fires when a single execution accumulates an
...
// the result. (grounding_failure, Tier 3) aggregates
...
// HumanInterventionPayload. One full HITL ask/answer
...
// ones and detectors recognize as well-known signals.
//
...
// AgentHandoffPayload. Captures the moment one agent
...
// graph. The handoff_kind discriminates the common shapes:
//
...
// detector for cascading_failure reads this alongside the
