...
export class HaltStreamReader {
    ...
    async _run() {
        ...
        try {
            ...
            if (!this.stopped) {
                ...
                console.debug("[mesedi.halt_stream] connect failed (fail-open)", { executionId: this.executionId, error: String(err) });
            }
            return;
        }
        if (resp.status !== 200) {
            console.debug("[mesedi.halt_stream] subscription rejected by backend", { executionId: this.executionId, status: resp.status });
        ...
        if (!resp.body) {
            console.debug("[mesedi.halt_stream] response has no body", { executionId: this.executionId });
            return;
        }
        ...
        try {
            ...
            if (!this.stopped) {
                console.debug("[mesedi.halt_stream] reader crashed (fail-open)", { executionId: this.executionId, error: String(err) });
            }
        }
        finally {
            try {
    ...
    _dispatchHalt(rawData) {
        ...
        try {
            ...
            console.debug("[mesedi.halt_stream] data was not JSON, using generic reason", { executionId: this.executionId, raw: rawData.slice(0, 200) });
        }
        try {
            this.onHalt(reason);
        }
...
//# sourceMappingURL=halt_stream.js.map