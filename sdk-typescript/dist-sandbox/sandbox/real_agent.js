...
// ── @tool examples ─────────────────────────────────────────────────
const searchWeb = tool({ name: "search_web" }, async (query) => {
    await new Promise((r) => setTimeout(r, 10));
    return [`result for ${query} #1`, `result for ${query} #2`, `result for ${query} #3`];
});
const calculator = tool({ name: "calculator" }, async (a, b, op = "+") => {
    if (op === "+")
        return a + b;
    if (op === "*")
        return a * b;
...
// ── @wrap examples ─────────────────────────────────────────────────
const agentWithTools = wrap({ name: "agent_with_tools" }, async (query) => {
    const results = await searchWeb(query);
    const product = await calculator(results.length, 10, "*");
    return `answer for ${query}: ${product} results scored`;
});
const agentThatCrashes = wrap({ name: "agent_that_crashes" }, async (query) => {
...
async function main() {
    ...
    try {
        ...
        console.log(`  wall-clock: ${ms(performance.now() - t)}`);
        console.log(`  caught (re-thrown by wrap, as expected): ${err.message}`);
    }
    ...
    console.log("  sqlite3 mesedi-dev.db \"SELECT execution_id, status, duration_ms, sdk_language FROM executions ORDER BY started_at DESC LIMIT 5;\"");
...
//# sourceMappingURL=real_agent.js.map