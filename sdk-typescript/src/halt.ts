...
/**
 * Per-execution budget envelope. Pass to `wrap({ budget: ... }, fn)`.
 * Any combination of fields can be set; unset fields impose no limit
 * on that axis.
 */
export interface Budget {
  /** Wall-clock seconds from execution start. Real elapsed time. */
  maxWallClockSeconds?: number;
  /**
   * Number of halt-safe boundary crossings allowed. Currently
   * incremented at `tool()` entry, `instrumentAnthropic`'s patched
   * `Messages.create` entry, and each `checkpoint()` call. Equivalent
   * of Python's `max_steps`.
   */
  maxSteps?: number;
  /**
   * Total input tokens across all LLM calls. Updated when the
   * Anthropic patch sees a successful response with token usage.
   */
  maxTokensIn?: number;
...
/**
 * Trigger that caused a halt, embedded in the crash_signature
 * (`halt:wall_clock`) so the dashboard groups halts by cause.
 */
export type HaltTrigger =
  | "wall_clock"
  | "step_count"
  | "token_total"
  | "remote_signal"; // reserved for sub-slice 21b SSE-driven halts
...
export class MesediHalt extends Error {
  ...
  readonly trigger: HaltTrigger;
  // Symbol-keyed marker, invisible to JSON.stringify, hard for user
  // code to spoof.
  readonly [MESEDI_HALT_MARKER]: true = true;

  constructor(reason: string, trigger: HaltTrigger) {
    super(reason);
 ...
 */
export function isMesediHalt(err: unknown): err is MesediHalt {
  if (!err || typeof err !== "object") return false;
  // Direct marker.
  if ((err as { [MESEDI_HALT_MARKER]?: boolean })[MESEDI_HALT_MARKER]) {
    return true;
  }
  ...
  while (cursor && safety < 10) {
    if (
      ...
      (cursor as { [MESEDI_HALT_MARKER]?: boolean })[MESEDI_HALT_MARKER]
    ) {
      return true;
    }
...
export class BudgetTracker {
  ...
  check(): void {
    ...
    if (this.budget.maxWallClockSeconds !== undefined) {
      ...
      if (elapsedSec >= this.budget.maxWallClockSeconds) {
        throw new MesediHalt(
          `wall-clock budget exceeded (${elapsedSec.toFixed(2)}s ≥ ${this.budget.maxWallClockSeconds}s)`,
          "wall_clock",
        );
      }
    }
    if (
      ...
      throw new MesediHalt(
        `step-count budget exceeded (${this.steps} ≥ ${this.budget.maxSteps})`,
        "step_count",
      );
    }
    if (this.budget.maxTokensIn !== undefined) {
      if (this.tokensIn >= this.budget.maxTokensIn) {
        throw new MesediHalt(
          `input-token budget exceeded (${this.tokensIn} ≥ ${this.budget.maxTokensIn})`,
          "token_total",
        );
      }
    }
    if (this.budget.maxTokensOut !== undefined) {
      if (this.tokensOut >= this.budget.maxTokensOut) {
        throw new MesediHalt(
          `output-token budget exceeded (${this.tokensOut} ≥ ${this.budget.maxTokensOut})`,
          "token_total",
        );
      }
    }
  ...
  addTokens(tokensIn: number, tokensOut: number): void {
    if (tokensIn > 0) this.tokensIn += tokensIn;
    if (tokensOut > 0) this.tokensOut += tokensOut;
  }
  ...
  get currentTokensIn(): number {
    return this.tokensIn;
  }
  get currentTokensOut(): number {
