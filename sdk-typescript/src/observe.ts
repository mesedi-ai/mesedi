...
// emitMcpCall moved to mcp.ts on 2026-09-10; re-exported so existing
// imports keep working.
export { emitMcpCall, type McpCallOptions } from "./mcp.js";
 ...
 * score over time. aggregates these into
 ...
 * downstream cascading_failure detector joins this event
 * back to the topology graph so that a handoff whose child
...
/**
 * Serializable handle to an in-flight HITL request.
 *
 ...
 * accepted but downstream HITL detectors (, ) only recognize
 ...
 * SLA expectation. The hitl_timeout detector reads this to
 * fire when the actual wait exceeds the configured SLA. Customers
 * without an explicit SLA can omit it and will fall back to a
