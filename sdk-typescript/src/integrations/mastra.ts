/**
 * Mastra integration for Mesedi (.B).
 *
...
//  user_message needs a taller cap than other state
...
// is bounded elsewhere (payload truncation applies on the
  ...
  private handleSpanEnded(span: AnyExportedSpan): void {
    ...
    switch (span.type) {
      ...
      case SPAN_WORKFLOW_RUN: {
        //  only the ROOT agent_run / workflow_run closes the
      ...
      case SPAN_WORKFLOW_STEP: {
        //  backend semantic_loop.G3 detector groups
...
/**
 *  extract the last user-role message's text from a
 ...
 *
 * Mastra shape observed live (probe):
 *   input = {
 *     messages: [
 *       {role: "system", content: "..."},
 *       {role: "user", content: [
...
/**
 *  extract the response text from a Mastra
...
function modelPayload(span: AnyExportedSpan): Record<string, unknown> {
  const s = span as unknown as {
    attributes?: {
      model?: string;
      provider?: string;
      //  Mastra 1.x MODEL_GENERATION spans carry per-call
  ...
  // Success path stays byte-identical with the pre-shape
  // to avoid churning consumers that already handle Mastra llm_calls.
  //  Mastra 1.x model_generation spans populate
  ...
  if (!hasError) {
    //  prefer per-call `usage` over ancestor-rollup
  ...
  //  failed-path fields matching the LangChain adapter's
  // contract so the provider_incident detector's canonical
...
function toolPayload(span: AnyExportedSpan): Record<string, unknown> {
  ...
  //  read span.errorInfo (Mastra 1.x's canonical error info
  // container on failed spans) — same class of bug modelPayload had
  // pre-. Backend tool_failures detector groups on
...
function mapStatus(span: AnyExportedSpan): Status {
  //  Mastra 1.x surfaces provider-level exceptions as
  ...
  // bug fixed for toolPayload/modelPayload. We still read
