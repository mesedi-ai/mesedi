/**
 * Unit tests for the Mastra exporter (.B).
 *
...
describe("MesediExporter — descendant spans", () => {
  test("MODEL_GENERATION emits llm_call with tokens", async => {
    ...
    //  status field distinguishes ok from failed.
    expect(evt.payload["status"]).toBe("ok");
  });

  // ──────────────────────────────────────────────────────────────────
  //  Mastra 1.x MODEL_GENERATION shape extraction
  // ──────────────────────────────────────────────────────────────────
  //
  // Live probe evidence (investigation):
  //   input = { messages: [
  //     {role:"system", content:"..."},
  ...
  // ──────────────────────────────────────────────────────────────────
  //  failed MODEL_GENERATION → llm_call with error_class
  // ──────────────────────────────────────────────────────────────────
  //
  ...
  // ──────────────────────────────────────────────────────────────────
  //  WORKFLOW_STEP emits `metadata` for semantic_loop
  // ──────────────────────────────────────────────────────────────────
  //
  // Live probe confirmed Mastra populates span.output
...
describe("MesediExporter — status + tenant + fail-open", () => {
  ...
  //  mapStatus now reads span.errorInfo (top-level) first —
...
// ──────────────────────────────────────────────────────────────────────
//  tool_failures + user_message cap
// ──────────────────────────────────────────────────────────────────────
...
describe("MesediExporter —  tool_failures errorInfo", () => {
  test("TOOL_CALL with errorInfo emits status=failed + exception fields", async => {
    ...
    const evt = getCaps().events[0]!;
    expect(evt.event_type).toBe("tool_call");
    //  pre-fix, this always shipped status="ok" because
