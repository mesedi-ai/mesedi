/**
 * End-to-end integration tests for the Mastra adapter (.B) —
...
// pattern shipped in Wave .
//
//  Mastra 1.x requires real `new Agent({...})` and
...
describe("Mastra × 20 detectors — end-to-end", () => {
  // 1. crashes (hybrid)
  //
  // reframe (mirrors LangChain crashes): 's mapStatus
  ...
  // workflow_run.errorInfo ('s workflow-step-throw grading
  ...
  //
  //  identical_call fires PER-EXECUTION (backend
  ...
  test("loops (similar_call): 3 near-duplicate prompts via Mastra", async => {
    ...
    await flush();
    //  near-duplicate content is a valid product signal in
  ...
  // 5. tool_failures —  uses real `createTool` from
  ...
  // 8. cost_velocity
  //
  //  cost_velocity absolute detector fires PER-EXECUTION
          ...
          steps: [
            {
              ...
              run: async (ctx: any) => {
                ...
                // TPM pacing carried forward — 1.5s
  ...
  //
  // reframe: real Anthropic sends consistent tool arg shapes
  ...
  //
  // reframe: real Anthropic (correctly, per safety training)
  ...
  //
  // reframe: the previous AbortController-with-1ms-deadline
