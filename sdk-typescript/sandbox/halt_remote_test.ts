...
async function triggerHaltAfter(executionId: string, delayMs: number): Promise<void> {
...
const runawayRemoteHaltAgent = wrap(
  { budget },
  async function runaway_remote_halt_agent(): Promise<string> {
    const ctx = currentExecutionContext();
    if (!ctx) throw new Error("must be called inside wrap()");
