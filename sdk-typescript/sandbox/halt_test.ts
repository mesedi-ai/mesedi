...
const slowTool = tool(async function slowTool(): Promise<string> {
...
const runawayWallClockBudget: Budget = { maxWallClockSeconds: 1.0 };
const runawayWallClockAgent = wrap(
  { budget: runawayWallClockBudget },
  async function runaway_wall_clock_agent(): Promise<string> {
...
const runawayStepCountBudget: Budget = { maxSteps: 3 };
const runawayStepCountAgent = wrap(
  { budget: runawayStepCountBudget },
  async function runaway_step_count_agent(): Promise<string> {
...
const cleanAgent = wrap(
  async function clean_agent(): Promise<string> {
...
async function main(): Promise<void> {
  ...
  console.log(
    "  Expect 2 halted rows from this run with crash_signature like halt:wall_clock and halt:step_count",
  );
}
